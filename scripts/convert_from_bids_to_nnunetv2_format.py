#!/usr/bin/env python3
"""
Prepares a new dataset for nnUNetv2, focusing on CARS segmentation.
    ...
    for subject in particpant_to_sample_dict.keys():
        for image in particpant_to_sample_dict[subject]:
            ...
            image_path = os.path.join(
                datapath, subject, "micr", f"{subject}_{image}_CARS.png"
            )
    ...
    for subject in particpant_to_sample_dict.keys():
        for image in particpant_to_sample_dict[subject]:
            ...
            label_path = os.path.join(
                ...
                "derivatives",
                "labels",
                f"{subject}_{image[:7]+"data"+image[7:]}_CARS_seg-{label_type}-manual.png",
            )
            label = np.round(
...
def main(args):
    ...
    dataset_info = {
        ...
        "labels": (
            {"background": 0, "myelin": 1, "axon": 2}
            if label_type == "axonmyelin"
            else (
                {"background": 0, "myelin": 1}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--DATASETNAME",
        default="CARS",
        help="Name of the new dataset, defaults to CARS",
    )
    parser.add_argument(
        "--DESCRIPTION",
        default="CARS segmentation dataset for nnUNetv2",
        help="Description of the new dataset, defaults to CARS segmentation dataset for nnUNetv2",
    )
    parser.add_argument(
        "--SPLITJSON",
        default="scripts/train_test_split.json",
